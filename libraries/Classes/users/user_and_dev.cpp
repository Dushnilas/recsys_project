...
const std::vector<Collection>& AllUsers::getAllCol() const{
    return _all_collection;
}

bool AllUsers::removeCol(Collection collection) {
    ...
    Logger::getInstance().logInfo("Collection " + collection.getName() + " doesn`t in the collections, " + _name);
    return false;
}

void AllUsers::createCol(const std::string& name) {
    int id = 0;
    _all_collection.emplace_back(id, name);
}
